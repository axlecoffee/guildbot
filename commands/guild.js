...
module.exports = {
    ...
    async execute(client, interaction) {
        if (interaction.options.getSubcommand() == 'checkuser') {
            ...
            db.collection('minecraft-accounts').findOne({ discord_id: discordUser.id }, function (err, res) {
                if (err) throw err;
                let inGameName;
                if (res == undefined) {
            ...
            await MongoClient.connect()
            const db = MongoClient.db()
            db.collection('hypixel-api-data').findOne({ sid: "guild-leaderboard-data" }, function (err, res) {
                if (err) throw err;
